...
import org.apache.flink.table.calcite.{FlinkParser, FlinkRelBuilder}
...
abstract class ExpressionTestBase {
  ...
  private def addSqlTestExpr(sqlExpr: String, expected: String): Unit = {
    ...
    val parsed = new FlinkParser(context._2.getParserConfig).parse(s"SELECT $sqlExpr FROM $tableName")
