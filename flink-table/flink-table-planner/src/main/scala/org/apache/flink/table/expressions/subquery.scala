...
case class In(expression: PlannerExpression, elements: Seq[PlannerExpression])
  ...
  override private[flink] def toRexNode(implicit relBuilder: RelBuilder): RexNode = {
    ...
    elements.head match {
      ...
      case TableReference(_, table: TableOperation) =>
        RexSubQuery.in(
          relBuilder.asInstanceOf[FlinkRelBuilder].flinkTableOperation(table).build(),
  ...
  override private[flink] def validateInput(): ValidationResult = {
    ...
    elements.head match {
      ...
      case TableReference(name, table: TableOperation) =>
