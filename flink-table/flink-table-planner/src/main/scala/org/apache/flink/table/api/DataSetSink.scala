...
@Internal
class DataSetSink[T](val outputType: TypeInformation[T]) extends BatchTableSink[T] {
  private var underlyingSet: DataSet[T] = _

  override def emitDataSet(dataSet: DataSet[T]): Unit = {
    underlyingSet = dataSet
  }
  ...
  def getUnderlyingSet: DataSet[T] = underlyingSet
}
