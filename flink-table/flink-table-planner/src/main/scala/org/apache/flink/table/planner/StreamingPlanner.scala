...
/**
  ...
  *                [[StreamingPlanner]].
  * @param config The [[TableConfig]] of this [[StreamingPlanner]].
  */
class StreamingPlanner(
  ...
  override def writeToSink[T](
    ...
    val relNode = relBuilder.tableOperation(inputTable).build()
    sink match {
  ...
  /**
    ...
    */
  override def explain(table: TableOperation, queryConfig: QueryConfig): String = {
    val ast = relBuilder.tableOperation(table).build()
    val optimizedPlan = optimize(ast, updatesAsRetraction = false)
    val dataStream = translateToCRow(optimizedPlan, queryConfig.asInstanceOf[StreamQueryConfig])
