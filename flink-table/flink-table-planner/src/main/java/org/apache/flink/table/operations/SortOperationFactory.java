...
public class SortOperationFactory {
	...
	public Limit createLimit(int offset, int fetch, TableOperation child) {
		...
		if (!(child instanceof Sort)) {
			throw new ValidationException("Limit operator must be preceded by an OrderBy operator.");
		}
		if (offset < 0) {
	...
	private void failIfStreaming() {
		if (isStreaming) {
			throw new ValidationException("Limit on stream tables is currently not supported.");
		}
	}
