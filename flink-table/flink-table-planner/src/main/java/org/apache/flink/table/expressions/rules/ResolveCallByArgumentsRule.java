...
/**
 * It checks if a {@link CallExpression} can work with given arguments. If the call expects different types of
 * arguments, but the given arguments have types that can be casted, a {@link BuiltInFunctionDefinitions#CAST}
...
final class ResolveCallByArgumentsRule implements ResolverRule {
	...
	private class CallArgumentsCastingVisitor extends ApiExpressionDefaultVisitor<Expression> {
		...
		public Expression visitCall(CallExpression call) {
			PlannerExpression plannerCall = context.bridge(call);
