 ...
 */
@Internal
public class AliasOperationFactory {

	private final AliasLiteralValidator aliasLiteralValidator = new AliasLiteralValidator();
	 ...
	 */
	public List<Expression> createAliasList(List<Expression> aliases, TableOperation child) {
	...
	private class AliasLiteralValidator extends ApiExpressionDefaultVisitor<ValueLiteralExpression> {
		...
		public ValueLiteralExpression visitValueLiteral(ValueLiteralExpression valueLiteralExpression) {
			String name = ApiExpressionUtils.extractValue(valueLiteralExpression, Types.STRING())
				.orElseThrow(() -> new ValidationException(
					"Alias only accept name expressions as arguments that is not '*'"));

			if (name.equals("*")) {
				throw new ValidationException("Alias only accept name expressions as arguments that is not '*'");
			}
		...
		protected ValueLiteralExpression defaultMethod(Expression expression) {
			throw new ValidationException("Alias only accept name expressions as arguments");
		}
