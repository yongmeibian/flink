 ...
 *
 * <p>The default set of rules ({@link ExpressionResolverBuilder#getDefaultRules()}) will resolve following references:
 * <ul>
...
public class ExpressionResolver {
	 ...
	 */
	public static ExpressionResolverBuilder resolverFor(TableReferenceLookup tableCatalog, TableOperation... inputs) {
	...
	public static class ExpressionResolverBuilder {
		...
		private final List<TableOperation> logicalNodes;
		...
		private List<ResolverRule> rules = new ArrayList<>(getDefaultRules());

		public static List<ResolverRule> getDefaultRules() {
			return Arrays.asList(
				ResolverRules.FLATTEN_STAR_REFERENCE,
				ResolverRules.OVER_WINDOWS,
				ResolverRules.FIELD_RESOLVE,
				ResolverRules.FLATTEN_CALL,
				ResolverRules.RESOLVE_CALL);
		}

		private ExpressionResolverBuilder(TableOperation[] logicalNodes, TableReferenceLookup tableCatalog) {
			this.logicalNodes = Arrays.asList(logicalNodes);
		...
		public ExpressionResolverBuilder appendCustomRule(ResolverRule rule) {
			this.rules.add(rule);
			return this;
		}

		public ExpressionResolver build() {
			return new ExpressionResolver(tableCatalog,
				new OperationFieldLookup(logicalNodes),
				logicalOverWindows,
				groupWindow,
				rules);
		}
