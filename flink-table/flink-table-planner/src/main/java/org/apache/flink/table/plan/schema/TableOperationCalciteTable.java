 ...
 * {@link TableOperationCalciteTable#getRowType(RelDataTypeFactory)}.
 */
@Internal
public class TableOperationCalciteTable extends AbstractTable implements TranslatableTable {
	...
	public static TableOperationCalciteTable createCalciteTable(TableOperationCatalogView catalogView) {
		return new TableOperationCalciteTable(catalogView, typeFactory -> {
			TableSchema tableSchema = catalogView.getSchema();
			return ((FlinkTypeFactory) typeFactory).buildLogicalRowType(tableSchema);
		});
	}

	private TableOperationCalciteTable(TableOperationCatalogView catalogView, RelProtoDataType rowType) {
