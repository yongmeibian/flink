...
class ExpressionsConsistencyCheckTest {
  ...
  @Test
  def testScalaStaticMethods(): Unit = {
  ...
  @Test
  def testScalaExpressionMethods(): Unit = {
  ...
  @Test
  def testJavaExpressionMethods(): Unit = {
    val scalaMethodNames = classOf[ImplicitExpressionConversions#WithOperations]
      .getMethods
      .map(_.getName)
      .toSet
  ...
  private def checkMethodsMatch(
    ...
    : Unit = {
    val missingJavaMethods = (checkedMethods -- methodsBeingCheckedAgainst)
      .filterNot(
        scalaName => {
    ...
    assertThat(missingJavaMethods.asJava, IsEmptyIterable.emptyIterableOf(classOf[String]))
  }
