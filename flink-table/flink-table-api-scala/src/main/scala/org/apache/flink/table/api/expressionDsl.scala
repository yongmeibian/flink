...
trait ImplicitExpressionConversions {
  ...
  /**
   * Creates a SQL INTERVAL literal corresponding to the given [[Duration]].
   */
  def interval(duration: Duration): Expression = Expressions.interval(duration)

  /**
   * Creates a SQL INTERVAL literal corresponding to the given [[Period]].
   */
  def interval(period: Period): Expression = Expressions.interval(period)
