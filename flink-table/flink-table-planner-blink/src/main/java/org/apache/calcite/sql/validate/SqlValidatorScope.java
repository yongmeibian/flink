 ...
 * object.
 *
 * <p>This class fixes a bug in Calcite in {@link SqlValidatorScope.Resolve}.
 * We should change the nullability of a resulting column only if the JOIN can produce
 * nullable results. We should not change the type if the scope does not add the nullability.
 */
public interface SqlValidatorScope {
   ...
   * {@link SelectScope}) which can be the
   * context for name-resolution. In scopes such as
   * {@link IdentifierNamespace}, it throws
  ...
  class Resolve {
     ...
     *
     * <p>FLINK CHANGE: otherwise leave the type as it was.
     */
    public RelDataType rowType() {
        if (nullable) {
            return namespace.getValidator().getTypeFactory()
                .createTypeWithNullability(namespace.getRowType(), true);
        } else {
