...
public class QueryOperationConverter extends QueryOperationDefaultVisitor<RelNode> {
	...
	private class SingleRelVisitor implements QueryOperationVisitor<RelNode> {
		...
		public RelNode visit(ValuesQueryOperation values) {
			RelDataTypeFactory.Builder builder = relBuilder.getTypeFactory().builder();
			for (TableColumn column : values.getTableSchema().getTableColumns()) {
				builder.add(column.getName(), relBuilder.getTypeFactory().createFieldTypeFromLogicalType(
					LogicalTypeDataTypeConverter.fromDataTypeToLogicalType(column.getType())
				));
			}
