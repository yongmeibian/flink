...
public class QueryOperationConverter extends QueryOperationDefaultVisitor<RelNode> {
	...
	private class AggregateVisitor extends ExpressionDefaultVisitor<AggCall> {
		...
		@Override
		public AggCall visit(CallExpression resolvedCall) {
			if (resolvedCall.getFunctionDefinition() == AS) {
				String aggregateName = extractValue(resolvedCall.getChildren().get(1), String.class)
						.orElseThrow(() -> new TableException("Unexpected name."));

				Expression aggregate = resolvedCall.getChildren().get(0);
				if (isFunctionOfKind(aggregate, AGGREGATE)) {
					return aggregate.accept(new AggCallVisitor(relBuilder, expressionConverter, aggregateName, false));
				}
			}
			throw new TableException("Expected named aggregate. Got: " + resolvedCall);
		}
