...
public class TableImpl implements Table {
	...
	public Table filter(Expression predicate) {
		Expression resolvedCallPredicate = predicate.accept(callResolver);
	...
	public OverWindowedTable window(OverWindow... overWindows) {
		...
		if (tableEnvironment.getClass().getSimpleName().equals("BatchTableEnvImpl")) {
	...
	private TableImpl createTable(QueryOperation operation) {
		return new TableImpl(tableEnvironment, operation, operationTreeBuilder, callResolver);
	}
	...
	private static final class GroupedTableImpl implements GroupedTable {
		...
		public Table select(Expression... fields) {
			List<Expression> expressionsWithResolvedCalls = Arrays.stream(fields)
				.map(f -> f.accept(table.callResolver))
				.collect(Collectors.toList());
			CategorizedExpressions extracted = OperationExpressionsUtils.extractAggregationsAndProperties(
				expressionsWithResolvedCalls
			);
			...
			return table.createTable(
				table.operationTreeBuilder.project(
					...
					table.operationTreeBuilder.tableAggregate(
						groupKey,
						tableAggregateFunction.accept(table.callResolver),
						table.operationTree
					)
				));
		}
