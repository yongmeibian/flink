 ...
 * @param <T>
 */
public class NFAFactoryCompiler<T> {
	...
	public static <T> NFAFactory<T> compileFactory(
		final Pattern<T, ? extends T> pattern,
		final TypeSerializer<T> inputTypeSerializer,
		boolean timeoutHandling) {
		if (pattern == null) {
			// return a factory for empty NFAs
			return new NFAFactoryImpl<>(inputTypeSerializer, 0, Collections.emptyList(), timeoutHandling);
		} else {
