...
public class SharedBufferTest extends TestLogger {
	...
	public void testSharedBuffer() throws Exception {
		...
		try (SharedBuffer<Event> sharedBuffer = new SharedBuffer<Event>(sharedBufferAccessor)) {
	...
	public void testSharedBufferCountersClearing() throws Exception {
		...
		try (SharedBuffer<Event> sharedBuffer =  new SharedBuffer<>(sharedBufferAccessor)) {
			...
			Iterator<Map.Entry<Long, Integer>> counters = sharedBufferAccessor.getEventCounters();
