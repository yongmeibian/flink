...
public class SystemResourcesCounter extends Thread {
	...
	public SystemResourcesCounter(Time logInterval) {
		logIntervalMs = logInterval.toMilliseconds();
		checkState(this.logIntervalMs > 0);

		setName(SystemResourcesCounter.class.getSimpleName() + " logging thread");
	...
	private void calculateNetworkUsage(NetworkIF[] networkIFs) {
		...
		for (int i = 0; i < networkIFs.length; i++) {
			...
			receiveRatePerInterface.set(i, (networkIF.getBytesRecv() - bytesReceivedPerInterface[i]) * 1000 / logIntervalMs);
			sendRatePerInterface.set(i, (networkIF.getBytesSent() - bytesSentPerInterface[i]) * 1000 / logIntervalMs);
