...
public abstract class AbstractFetcher<T, KPH> {
	 ...
	 */
	protected void emitRecord(T record, KafkaTopicPartitionState<KPH> partitionState) throws Exception {
		if (record != null) {
			if (timestampWatermarkMode == NO_TIMESTAMPS_WATERMARKS) {
				...
				emitRecordWithTimestampAndPeriodicWatermark(record, partitionState, Long.MIN_VALUE);
			} else {
				emitRecordWithTimestampAndPunctuatedWatermark(record, partitionState, Long.MIN_VALUE);
			}
		}
	}

	protected void finalizeOffset(KafkaTopicPartitionState<KPH> partitionState, long offset) {
	...
	protected void emitRecordWithTimestamp(
			T record, KafkaTopicPartitionState<KPH> partitionState, long timestamp) throws Exception {
		...
		if (record != null) {
			if (timestampWatermarkMode == NO_TIMESTAMPS_WATERMARKS) {
				...
				emitRecordWithTimestampAndPeriodicWatermark(record, partitionState, timestamp);
			} else {
				emitRecordWithTimestampAndPunctuatedWatermark(record, partitionState, timestamp);
			}
		}
	}
	...
	private void emitRecordWithTimestampAndPeriodicWatermark(
			T record, KafkaTopicPartitionState<KPH> partitionState, long kafkaEventTimestamp) {
		@SuppressWarnings("unchecked")
