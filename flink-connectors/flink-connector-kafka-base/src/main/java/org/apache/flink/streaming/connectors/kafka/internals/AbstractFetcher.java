...
public abstract class AbstractFetcher<T, KPH> {
	 ...
	 *
	 * @param records The record to emit
	...
	protected void emitRecordsWithTimestamps(
		...
		synchronized (checkpointLock) {
			for (StreamRecord<T> record : records) {
				sourceContext.collectWithTimestamp(record.getValue(), record.getTimestamp());
			}
			partitionState.setOffset(offset);
		}

		if (timestampWatermarkMode == PUNCTUATED_WATERMARKS) {
			emitPunctuatedWatermark(records, partitionState);
		}
	}
	...
	private void emitPunctuatedWatermark(
		...
		for (StreamRecord<T> record : records) {
			Watermark recordWatermark = withWatermarksState.checkAndGetNewWatermark(
				record.getValue(),
				record.getTimestamp());
			if (recordWatermark != null) {
				newWatermark = recordWatermark;
			}
		}
